...
import { StockChartServiceAGateway } from './stock-chart-service-a.gateway';
import { DownstreamChartPoint } from '../dto/downstream-chart.dto';

jest.mock('axios');
const mockedAxios = axios as jest.Mocked<typeof axios>;

describe('StockChartServiceAGateway', () => {
  let gateway: StockChartServiceAGateway;
  ...
  beforeEach(() => {
    ...
    const configService = {
      get: jest.fn().mockReturnValue('http://service-a.example.com'),
    } as unknown as ConfigService;

    gateway = new StockChartServiceAGateway(configService);
  });
  ...
  describe('getStockChart', () => {
    it('正常系: DownstreamChartPoint が StockChartPoint に変換される', async () => {
      ...
      const downstream: DownstreamChartPoint[] = [
        { date: '2025-10-01', price_jpy: 300000 },
        { date: '2025-11-01', price_jpy: 320000 },
      ];
      ...
      expect(result[0].priceJpy).toBe(300000);
      expect(result[1].date).toBe('2025-11-01');
      expect(result[1].priceJpy).toBe(320000);
