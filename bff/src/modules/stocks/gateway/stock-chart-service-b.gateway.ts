...
@Injectable()
export class StockChartServiceBGateway implements StockChartGatewayPort {
  private readonly axiosInstance: AxiosInstance;
  private readonly logger = new Logger(StockChartServiceBGateway.name);
  ...
  async getStockChart(name: string, from: string, to: string): Promise<StockChartPoint[]> {
    try {
      ...
      this.logger.log(`Received ${response.data.length} chart points from Service B`);
      ...
      this.logger.error('StockChartServiceBGateway failed', error);
      throw new InternalServerErrorException('Stock chart service B is unavailable');
    }
  }
}
